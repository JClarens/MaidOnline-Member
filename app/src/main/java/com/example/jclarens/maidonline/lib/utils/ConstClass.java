package com.example.jclarens.maidonline.lib.utils;

/**
 * Created by Zackzack on 15/07/2017.
 */

public class ConstClass {
    //Shared Preference
    public static String USER = "user";
    //extra
    public static String REGISTER_EXTRA = "register";
    public static String LOGIN_EXTRA = "login";

    public static String PLACES_EXTRA = "places";
    public static String LANGUAGES_EXTRA = "languages";
    public static String JOBS_EXTRA = "jobs";
    public static String WTS_EXTRA = "WTs";
    public static String ADDITIONALINFOS_EXTRA = "additionalInfos";
    public static String WALLETS_EXTRA = "wallets";
    public static String ART_EXTRA = "art";
    public static String ORDER_EXTRA = "order";
    public static String OFFER_EXTRA = "offer";
    public static String MYTASK_EXTRA = "mytask";
    public static String EMERGENCY_EXTRA = "emergency";

    //pager
    public static String PAGER_PESAN_POS = "pesanpos";
    public static String PAGER_TRANSAKSI_POS = "transaksipos";

}
